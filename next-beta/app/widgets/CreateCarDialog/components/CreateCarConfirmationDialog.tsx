'use client'

import {useCreateCarConfirmationDialogUI} from "../hooks/useCreateCarConfirmationDialogUI";

export const CreateCarConfirmationDialog = () => {
    const {
        onCreateCarConfirmationDialogConfirmHandler,
        onCreateCarConfirmationDialogCancelHandler,
        isCreateCarConfirmationDialogOpen
    } = useCreateCarConfirmationDialogUI();

    if(!isCreateCarConfirmationDialogOpen) return null;

    return (
        <div className="modal modal-open">
            <div className="modal-box relative">
                <h3 className="font-bold text-lg mb-5">Edit car</h3>
                <p className="py-4">Form contains unsaved data. Please confirm action.</p>
                <div className="modal-action">
                    <button className="btn btn-secondary" onClick={onCreateCarConfirmationDialogCancelHandler}>Cancel</button>
                    <button className="btn" onClick={onCreateCarConfirmationDialogConfirmHandler}>Confirm</button>
                </div>
            </div>
        </div>
    )
}
